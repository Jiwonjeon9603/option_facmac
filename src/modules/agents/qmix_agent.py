...
class ActQMIXRNNAgent(nn.Module):
    def __init__(self, input_shape, args):
        super(ActQMIXRNNAgent, self).__init__()
        self.args = args
    ...
    def forward(self, inputs, hidden_state):
        x_inputs = inputs.reshape(-1, inputs.shape[-1])
        x = F.relu(self.fc1(x_inputs))
        h_in = hidden_state.reshape(-1, self.args.rnn_hidden_dim)
        h = self.rnn(x, h_in)
        q = self.fc2(h)
        return q, h

class OptQMIXRNNAgent(nn.Module):
    def __init__(self, input_shape, args):
        super(OptQMIXRNNAgent, self).__init__()
        self.args = args
